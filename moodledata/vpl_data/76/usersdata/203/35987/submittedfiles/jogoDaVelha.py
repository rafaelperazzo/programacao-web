# -*- coding: utf-8 -*-
import math

x1 = int(input('Digite x1: '))
x2 = int(input('Digite x2: '))
x3 = int(input('Digite x3: '))
x4 = int(input('Digite x4: '))
x5 = int(input('Digite x5: '))
x6 = int(input('Digite x6: '))
x7 = int(input('Digite x7: '))
x8 = int(input('Digite x8: '))
x9 = int(input('Digite x9: '))

#CONTINUE...
if (x1==1 and x4==1) or (x1==1 and x5==1) or (x1==1 and x2==1):
        print('1')
elif (x1==0 and x4==0) or (x1==0 and x5==0) or (x1==0 and x2==0):
        print('0')
elif (x2==1 and x5==1):
        print('1')
elif (x2==0 and x5==0):
        print('0')
elif (x3==1 and x6==1) or (x3==1 and x5==1):
        print('1')
elif (x3==0 and x6==0) or (x3==1 and x5==1):
        print('0')
else:
    print('E')