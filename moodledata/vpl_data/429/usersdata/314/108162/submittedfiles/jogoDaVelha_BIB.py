...
def solicitaSimboloDoHumano() :
    ...
    while True :
        simbolo = input('\nQual símbolo você deseja utilizar no jogo? ').upper().strip()
...
#Preenche no tabuleiro a posiçao escolhida pelo usuario, retornando o tabuleiro preenchido.
def jogadaHumana(tabuleiro, simbolo, jogada) :
    aux = jogada.split()
    tabuleiro[int(aux[0])][int(aux[1])] = simbolo
    return tabuleiro

# Jogada do computador (posição vazia aleatória), retornando o tabuleiro preenchido.
def jogadaComputador(tabuleiro, simbolo):
    ...
    while True:
        ...
        if tabuleiro[jogada[0]][jogada[1]] == ' ' :
            ...
            break
    return tabuleiro
#Verifica se a jogada do usuario é valida 
...
# Verifica se houve vencedor ou o jogo terminou empatado
