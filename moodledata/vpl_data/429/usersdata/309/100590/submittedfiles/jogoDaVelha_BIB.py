...
def solicitaSimboloDoHumano():
  #  nome=input('Qual seu nome(ou apelido)? ')
    simbH= (str.upper((input("Qual o simbolo que você deseja utilizar no jogo? "))))
    while simbH!="X" and simbH!="O":
...
def validarJogada(nome, simbH, tabuleiro, i, j):
    if tabuleiro[i][j]!="X" and tabuleiro[i][j]!="x" and tabuleiro[i][j]!="o" and tabuleiro[i][j]!="O" :
        tabuleiro[i][j]=simbH
