...
def jogadaHumana(nome, simbH, tabuleiro):
    ...
    while (i!="2" and i!="1" and i!="0") or (j!="2" and j!="1" and j!="0"):
        print('\nOps! Jogada invalida... ')
        casa=input("\n Qual a sua jogada, %s ?" %nome)
        i=(casa[0])
        j=(casa[2])
...
#código limpo da função que verifica o vencedor 
def VerificaVencedor(tab, simbH, nome, simbM):
    x=1
    
        
    if tab[0][0]==tab[1][1] and tab[1][1]==tab[2][2] and tab[0][0]==tab[2][2]:
        if tab[0][0]==simbH:
            x=2
        elif tab[0][0]==simbM:
            x=4
    ...
    for i in range (3):
        ...
        if tab[i][0]==tab[i][1] and tab[i][0]==tab[i][2] and tab[i][1]==tab[i][2]:
            if tab[i][0]==simbH:
                x=2
            elif tab[i][0]==simbM:
                x=4
            i=3
        elif tab[0][i]==tab[1][i] and tab[0][i]==tab[2][i] and tab[1][i]==tab[2][i]:
            if tab[0][i]==simbH:
                x=2
            elif tab[0][i]==simbM:
                x=4
