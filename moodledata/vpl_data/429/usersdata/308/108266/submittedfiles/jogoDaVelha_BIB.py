...
from copy import deepcopy as clonar
def verificaVencedor(tabela): #Verifica se há vencedor || Parâmetro: Recebe a tabela para verificar || Retorno: Retorna o simbolo do vencedor
    for a in range(0, 3):
        if (tabela[0][a]==tabela[1][a]==tabela[2][a]) and not(tabela[1][a]==' '):
            return tabela[1][a]
        elif (tabela[a][0]==tabela[a][1]==tabela[a][2]) and not(tabela[a][1]==' '):
            return tabela[a][1]
    if ((tabela[0][0]==tabela[1][1]==tabela[2][2])and not(tabela[1][1]==' ')) or ((tabela[2][0]==tabela[1][1]==tabela[0][2])and not(tabela[1][1]==' ')):
        return tabela[1][1]
    return ' '
...
def sorteioPrimeiraJogada(nomes): #Sorteio || Parâmetro: Nome dos jogadores e tabela || Retorno: Retorna o resultado do sorteio (0 ou 1)
    ...
    return resultado
def validaJogada(jogada, visual): #Valida jogada || Parâmetro: Nome dos jogadores e tabela || Retorno: Verdadeiro ou falso
    try: 
        return not visual[int(jogada[0])][int(jogada[2])]==' '
    except:
        return True
...
def jogadaComputador(nomes, tabela): #Jogada do computador || Parâmetro: Nome dos jogadores e tabela || Retorno: Tabela modificada 
    if tabela[1][1]==nomes[1][1]:
        jogadasValidas = ['0 1', '1 0', '1 2', '2 1', '1 1', '0 0', '2 2', '0 2', '2 0']
    else:
        jogadasValidas = ['0 0', '2 2', '0 2', '2 0','0 1', '1 0', '1 2', '2 1', '1 1']
