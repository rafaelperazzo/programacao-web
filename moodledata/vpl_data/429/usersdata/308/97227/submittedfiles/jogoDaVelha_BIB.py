...
def verificaVitoria(jogada, tabela):
    visuala = tabela
    if validaJogada(jogada, visuala):
            visuala[int(jogada[0])][int(jogada[2])] = nomes[0][1]
            if verificaVencedor(visuala) != ' ': 
                return True
            else:
                return False
            visuala[int(jogada[0])][int(jogada[2])] = nomes[1][1]
            if verificaVencedor(visuala) != ' ': 
                return True
            else:
                return False
...
def jogadaComputador(nomes, tabela): #Jogada do computador || Parâmetro: Nome dos jogadores e tabela || Retorno: Tabela modificada 
    for teste in ['0 0', '0 1', '0 2', '1 0', '1 1', '1 2', '2 0', '2 1', '2 2']:
        if verificaVitoria(teste, tabela):
            jogada = teste 
