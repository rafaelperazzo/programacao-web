...
def jogadaComputador(nomes, tabela): #Jogada do computador || Parâmetro: Nome dos jogadores e tabela || Retorno: Tabela modificada 
    ...
    for i in range(0, 3):
        for j in range(0, 3):
            ...
            if not validaJogada(('%d %d' % (i, j)), copia):
                copia[i][j] = nomes[0][1]
                if verificaVencedor(copia)== nomes[0][1]:
                    ...
                    for k in range(0, 3):
                        for l in range(0, 3):
                            ...
                            if not validaJogada(('%d %d' % (k, l)), copia):
                                copia[i][j] = nomes[0][1]
                                if verificaVencedor(copia)== 'X':
                                    tabela[i][j] = nomes[1][1]
                                    return tabela
                copia[i][j] = nomes[1][1]
                if verificaVencedor(copia)== nomes[1][1]:
                    tabela[i][j] = nomes[1][1]
                    return tabela
                else: 
                    for k in range(0, 3):
