...
def jogadaInteligente(tabelacomputador):
    for jogada in ['0 0', '0 1', '0 2', '1 0', '1 1', '1 2', '2 0', '2 1', '2 2']:
        copia = tabelacomputador
        if validaJogada(jogada, copia):
            copia[int(jogada[0])][int(jogada[2])] = 'X'
            if verificaVencedor(copia)!= ' ':
                return jogada
    jogada = ('%d %d' % (randint(0, 2), randint(0, 2)))
    while validaJogada(jogada, tabelacomputador):
        jogada = ('%d %d' % (randint(0, 2), randint(0, 2)))
    return jogada
   
def jogadaComputador(nomes, tabela): #Jogada do computador || Parâmetro: Nome dos jogadores e tabela || Retorno: Tabela modificada 
    jogada = jogadaInteligente(tabela)
