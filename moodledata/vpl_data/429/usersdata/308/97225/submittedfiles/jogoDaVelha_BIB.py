...
def verificaVitoria(nomes, tabela):
    jogadaa='0'
    for teste in ['0 0', '0 1', '0 2', '1 0', '1 1', '1 2', '2 0', '2 1', '2 2']:
        visuala = tabela
        visualb = tabela
        if validaJogada(teste, visuala):
            visuala[int(teste[0])][int(teste[2])] = nomes[0][1]
            visualb[int(teste[0])][int(teste[2])] = nomes[1][1]
            if verificaVencedor(visuala) != ' ' or verificaVencedor(visualb) != ' ': 
                jogadaa = teste
                break
    return jogadaa
   
def jogadaComputador(nomes, tabela): #Jogada do computador || Parâmetro: Nome dos jogadores e tabela || Retorno: Tabela modificada 
    jogadaa = verificaVitoria(nomes,tabela*2)
    print(id(tabela))
    if  jogadaa!= '0':
        tabela[int(jogadaa[0])][int(jogadaa[2])] = nomes[1][1]
    else:
    jogadaa = ('%d %d' % (randint(0, 2), randint(0, 2)))
    while validaJogada(jogadaa, tabela):
        jogadaa = ('%d %d' % (randint(0, 2), randint(0, 2)))
    tabela[int(jogadaa[0])][int(jogadaa[2])] = nomes[1][1]
    return tabela
