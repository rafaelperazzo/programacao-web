...
def verificaVitoria(nomes, tabela):
    jogadaa='0'
    for teste in ['0 0', '2 2', '2 0', '0 1', '0 2', '1 0', '1 1', '1 2', '2 1']:
        visuala = tabela
        visualb = tabela
        if validaJogada(teste, visuala):
            visuala[int(teste[0])][int(teste[2])] = nomes[1][1]
            visualb[int(teste[0])][int(teste[2])] = nomes[0][1]
...
def jogadaComputador(nomes, tabela): #Jogada do computador || Parâmetro: Nome dos jogadores e tabela || Retorno: Tabela modificada 
    jogadaa = verificaVitoria(nomes,tabela)
    if  jogadaa!= '0':
