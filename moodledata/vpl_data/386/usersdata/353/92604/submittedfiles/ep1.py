...
def baskara(a,b,c):
    ...
    if delta>0:
        ...
        x11=x1+x3
        x22=x2-x3 
        return('reais simples', '%4.0f'%x11, '%4.0f'%x22)
    elif delta==0:
        return('real dupla', '%4.0f'%x1, '%4.0f'%x2)
    else:
        delta=delta*(-1)
        x3=((raiz2(delta,epsilon))/(2*a))       
        x1=((-b)/(2*a))
        x2=((-b)/(2*a))
        return('complexas'), (complex(x1,x3)), (complex(x2,-x3))
...
for equação in range(0,nequacoes,1):
    ...
    if a!=0:
        z=baskara(a,b,c)
        print('%.2f'% a), print('%.2f'% b), print('%.2f'% c), print(z)
    else:
