...
while (n>=contador):
    ...
    if (delta>0) :
        ...
        resposta_da_raiz = raiz (delta,eps)
        x1_parte_real =(-b)/(2*a)
        x1_parte_imaginaria = resposta_da_raiz/(2*a)
        x2_parte_real =(-b)/(2*a)
        x2_parte_imaginaria =resposta_da_raiz/(2*a)
        print('%.2f %.2f %.2f complexas %.4f + %.4fi %.4f - %.4fi' % (a,b,c,x1_parte_real,x1_parte_imaginaria,x2_parte_real,x2_parte_imaginaria))
