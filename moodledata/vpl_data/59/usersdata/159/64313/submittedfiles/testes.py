...
def graus (azi):
    a=np.zeros((len(azi),3))    #criou-se uma matriz com tres colunas para guardar graus,minutos e segundos
   '''
   Dentro dessa repetição a gente determinou que o grau seria a parte inteira do dado de entrada,chamamos de b o resto 
   do azimute da entrada e c esse resto multiplicado por 60 para transformar em minutos ja que o que restou nao daria para formar
   um grau.Depois disso para encontrar os minutos pegamos a parte inteira e o que sobrou que chamamos de j novamente multiplicamos 
   por 60 para transformar em segundos.Assim retornamos a matriz guardando nela graus,minutos e segundos de um azimute cada um em
