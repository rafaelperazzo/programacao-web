...
#Primeira função que será utilizada para encontrar o novo valor de "D" caso fn seja diferente de f:
...
if fn!=f:
    ...
    def definicao_do_D(f, g, v, Q, dH, L,):
        for i in range(0 , k , 1):
            D = ((8*f*L*(Q**2))/(dH*(math.pi**2)*g))**(1/5)
            Rey = (4*Q)/(math.pi*D*v)
            fn = (((64/Rey)**8)+(95*(math.log((e/3.7*D)+(5.47/(Rey**0.9))))-(2500/Rey)**6)**-16)**0.125
            f = fn
        return D

#Segunda função que será utilizada para encontrar o novo valor de "f" caso fn seja diferente de f:
...
'''
Feitas as funções que irão definir os novos possíveis valores para "f" e "D", é preciso decidir a quantidade
de casas decimais para o resultado e fazer o print.
'''


        
    print ('D será igual a: %.4f'%definicao_do_D(f,g,v,Q,dH,L,))
    print ('f será igual a: %.4f'%definicao_do_f(f,g,v,Q,dH,L,))
