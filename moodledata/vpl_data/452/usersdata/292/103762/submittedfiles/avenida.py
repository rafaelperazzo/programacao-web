...
class Quadras(object):
    def __init__(self, M, N):
        ...
        for i in range(1, M+1):
            ...
            for j in range(1, N+1):
                q = int(input("Digite o valor (em milhões de reais) da %dº quadra no sentido Norte-sul que também é a %dº no sentido Leste-oeste: "%(i,j)))
                
                while True:
                    if 1 <= q <= 100:
                        break
                    q = int(input("Valor fora da faixa! Digite novamente: "))
    ...
    def minimizaPreco(self):
        ...
        for i in range(0, self.norte_sul):
            soma_colunas = 0
            
            for j in range(0, self.leste_oeste):
