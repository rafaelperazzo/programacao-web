...
def simbolo_escolhido ():
    simb = str(input("Qual símbolo você deseja utilizar no jogo? "))
    while (simb!="X" and simb!="O"):
        print("Símbolo inválido! Tente novamente.")
        simb = str(input("Qual símbolo (X ou O) você deseja utilizar no jogo? "))
    return simb
...
def jogadaHumana_2 (nome,s,m,cont,i,j):
...
def sorteio_i ():
    global sort_i
    sort_i = random.randint(0,2)
    return sort_i
    
def sorteio_j ():
    global sort_j 
    sort_j = random.randint(0,2)
    return sort_j
