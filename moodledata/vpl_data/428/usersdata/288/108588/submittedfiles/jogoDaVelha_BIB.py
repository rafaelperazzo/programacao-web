...
def jogadaHumana(tabuleiro):
    movimento = 0
    while movimento not in '1 2 3 4 5 6 7 8 9'.split() or not vazio(tabuleiro, int(movimento)): #VALIDAJOGADA
        movimento=input("\nQual a sua jogada? ") 
        if movimento == "0 0":
            movimento= "7"
        elif movimento== "0 1":
...
def validajogada(tabuleiro, simbolo, movimento): #COMPLEMENTO_VALIDAJOGADA
...
def jogadaComputador(tabuleiro, simboloComputador):
    ...
    for i in range(1,10):
        ...
        if vazio(mostra, i):
            validajogada(mostra, simboloComputador, i)
    ...
    for i in range(1, 10):
        mostra = mostraTabuleiro(tabuleiro)
        if vazio(mostra, i):
            validajogada(mostra, simboloVoce, i)
