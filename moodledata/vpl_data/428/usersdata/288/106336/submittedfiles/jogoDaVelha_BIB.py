...
def solicitaSimboloDoHumano():
    ...
    while (simbolo)!= 'X' and (simbolo)!='O':
        ...
        simbolo=str(input("Escolha um simbolo CORRETO para jogar! (X,O): "))
    if simbolo=="X":
        return ['X','O']
    else:
        return ['O','X']
...
def jogadaHumana(tabuleiro):
    ...
    while movimento not in '1 2 3 4 5 6 7 8 9'.split() or not vazio(tabuleiro, int(movimento)):
        movimento=input("Qual a sua jogada?")
...
def jogadaComputador(tabuleiro, simboloComputador):
    ...
    for i in range(1,10):
        ...
        if vazio(mostra, i):
            movimentacao(mostra, simboloComputador, i)
    ...
    for i in range(1, 10):
        mostra = mostraTabuleiro(tabuleiro)
        if vazio(mostra, i):
            movimentacao(mostra, simboloVoce, i)
