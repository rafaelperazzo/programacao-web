...
def soma(a,b):
    c = np.zeros( (ashape[0],a.shape[1]) )
    for i in range(0,c.shape[0],1):
        for j in range(0,c.shape[1],1):
            c[i,j] = a[i,j] + b[i,j]
    return c

#UM PROGRAMA EM PYTHON QUE FAÇA A SOMA DE 2 MATRIZES. MOSTRAR A MATRIZ C = A + B

#PASSO 1: PREPARAR E CRIAR AS MATRIZES. As matrizes possuem mesma dimensão.
