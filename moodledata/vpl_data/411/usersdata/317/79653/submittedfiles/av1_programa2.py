# -*- coding: utf-8 -*-
v =foat(input('digite o valor do produto')
print('o valor de {}'. format(v))

valor1 = (valor*0,15)+valor
print(Á Vista em dinheiro ou cheque, recebe {} de desconto'.format ( valor1))
codigo2 = (valor*,10)+valor
print(Á Vistano no cartão de crédito,recebe {} de desconto'.format ( codig2))
codigo3 = valor
print(Em duas vezes, preço {} normal de etiqueta sem juros'.format (codigo3))
codigo4=(valor*,10)*valor
print(Em duas vezes, preço  norma de etiqueta mais juros de {}'.format(codigo4))

