# -*- coding: utf-8 -*-
# COLOQUE SEU PROGRAMA A PARTIR DAQUI
from minha_bib import verificar_vitoria
from minha_bib import sorteio
from minha_bib import sorteio2
from minha_bib import maquinainteligente
from minha_bib import maquinainteligente2
from minha_bib import mostrar_tabuleiro
from minha_bib import jogadausuario
from minha_bib import simbolo
import time
tabuleiro=[[1,2,3],[1,2,3],[1,2,3]]
for i in range(0,3,1):
    for j in range(0,3,1):
        tabuleiro[i][j]=" "
print('------------------------------------------------')
print('JOGO DA VELHA')
print('Olá\nSeja Bem Vindo ao jogo da velha do grupo G(SIVINI, EDUARDO E NICHOLAS) !')
nome1=str(input('Qual seu nome(ou apelido)? '))
s1=simbolo(nome1)
s2=0
if s1=='X':
    s2='O'
else:
    s2='X'
print('Ok, o símbolo que você vai utilizar é ',s1,'\nAgora eu vou te explicar como vai funcionar o jogo, esse é o nosso tabuleiro: ')
time.sleep(1)
mostrar_tabuleiro(tabuleiro)
time.sleep(3)
print('Você vai me informar a casa que quer jogar com números.\nE cada um desses números representa as seguintes casas:')
time.sleep(1)
print('00 | 01 | 02\n10 | 11 | 12\n20 | 21 | 22')
time.sleep(3)
print('E aí eu vou lá e substituo a casa pelo seu símbolo, por exemplo:\nVocê me informa a seguinte jogada: 22')
print('Eu vou lá e...')
time.sleep(3)
print('',tabuleiro[0][0],'|',tabuleiro[0][1],'|',tabuleiro[0][2],'\n',tabuleiro[1][0],'|',tabuleiro[1][1],'|',tabuleiro[1][2],'\n',tabuleiro[2][0],'|',tabuleiro[2][1],'|',s1,)
time.sleep(2)

começar=str(input('Entendeu? Vamos começar?[S/N] '))
print('---------------------------------------------------')
time.sleep(1)
while True:
    if começar in 'SN':
        break
    else:
        print('Resposta inválida')
        começar=str(input('Vamos começar?[S/N] '))
        continue
#INICIO DO JOGO
while começar=='S':
    tabuleiro=[[1,2,3],[1,2,3],[1,2,3]]
    for i in range(0,3,1):
        for j in range(0,3,1):
            tabuleiro[i][j]=" "
    tipo=str(input('Escolha o tipo de jogo que você quer jogar:\nTecle 1 para PlayerVSPlayer\nTecle 2 para PlayerVSMáquina FÁCIL\nTecle 3 para PlayerVSMáquina MÉDIO\nTecle 4 para PlayerVSMáquina DIFÍCIL\n'))
    while True:
        if tipo in str(1234):
            break
        else:
            tipo=str(input('Selecione um tipo de jogo válido: '))
            continue
    if tipo=='1':
        nome2=str(input('Qual o nome do segundo jogador? '))
        inicio=sorteio(0,1)
        if inicio==0:
            inicio=str(nome1)
        else:
            inicio=str(nome2)
        print('O vencedor do sorteio para início foi '+inicio)
        time.sleep(1)
        if inicio==nome1:
            print('Então, '+nome1+' começa')
            for k in range(1,10,1):
                if k%2!=0:
                    tabuleiro=jogadausuario(nome1,tabuleiro,s1)
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('PARABÉNS '+nome1+',VOCÊ VENCEU')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
                elif k%2==0:
                    print('Sua vez '+nome2+'.')
                    tabuleiro=jogadausuario(nome2,tabuleiro,s2)
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('PARABÉNS '+nome2+',VOCÊ VENCEU')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
        elif inicio==nome2:
            print('Então, '+nome2+' começa.')
            for k in range(1,10,1):
                if k%2!=0:
                    jogadausuario(nome2,tabuleiro,s2)
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('PARABÉNS '+nome2+',VOCÊ VENCEU')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
                elif k%2==0:
                    jogadausuario(nome1,tabuleiro,s1)
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('PARABÉNS '+nome1+' ,VOCÊ VENCEU')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
    elif tipo=='2':
        inicio=sorteio(0,1)
        if inicio==0:
            inicio=str('Usuário')
        else:
            inicio=str('Máquina')
        print('O vencedor do sorteio para incio foi '+inicio)
        time.sleep(1)
        if inicio=='Usuário':
            print('Então você começa')
            for k in range(1,10,1):
                if k%2!=0:
                    tabuleiro=jogadausuario(nome1,tabuleiro,s1)
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('PARABÉNS,VOCÊ VENCEU')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
                elif k%2==0:
                    print('Minha vez')
                    time.sleep(2)
                    i=int(sorteio2(0,2))
                    j=int(sorteio2(0,2))
                    while tabuleiro[i][j]!=" ":
                        i=int(sorteio2(0,2))
                        j=int(sorteio2(0,2))
                    tabuleiro[i][j]=s2
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('Ahh, não foi dessa vez')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
        elif inicio=='Máquina':
            print('Então eu começo')
            for k in range(1,10,1):
                if k%2!=0:
                    print('Minha vez')
                    time.sleep(2)
                    i=int(sorteio2(0,2))
                    j=int(sorteio2(0,2))
                    while tabuleiro[i][j]!=" ":
                        i=int(sorteio2(0,2))
                        j=int(sorteio2(0,2))
                    tabuleiro[i][j]=s2
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('Ahh, não foi dessa vez')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
                elif k%2==0:
                    tabuleiro=jogadausuario(nome1,tabuleiro,s1)
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('PARABÉNS,VOCÊ VENCEU')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
    elif tipo=='3':
        inicio=sorteio(0,1)
        if inicio==0:
            inicio=str('Usuário')
        else:
            inicio=str('Máquina')
        print('O vencedor do sorteio para incio foi '+inicio)
        time.sleep(1)
        if inicio=='Usuário':
            print('Então você começa')
            for k in range(1,10,1):
                if k%2!=0:
                    tabuleiro=jogadausuario(nome1,tabuleiro,s1)
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('PARABÉNS,VOCÊ VENCEU')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
                elif k%2==0:
                    print('Minha vez')
                    time.sleep(2)
                    x=str(maquinainteligente(tabuleiro))
                    i=int(x[0])
                    j=int(x[1])
                    while tabuleiro[i][j]!=" ":
                        x=str(maquinainteligente(tabuleiro))
                        i=int(x[0])
                        j=int(x[1])
                    tabuleiro[i][j]=s2
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('Ahh, não foi dessa vez')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
        elif inicio=='Máquina':
            print('Então eu começo')
            for k in range(1,10,1):
                if k%2!=0:
                    print('Minha vez')
                    time.sleep(2)
                    x=str(maquinainteligente(tabuleiro))
                    i=int(x[0])
                    j=int(x[1])
                    while tabuleiro[i][j]!=" ":
                        x=str(maquinainteligente(tabuleiro))
                        i=int(x[0])
                        j=int(x[1])
                    tabuleiro[i][j]=s2
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('Ahh, não foi dessa vez')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
                elif k%2==0:
                    tabuleiro=jogadausuario(nome1,tabuleiro,s1)
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('PARABÉNS,VOCÊ VENCEU')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
    elif tipo=='4':
        inicio=sorteio(0,1)
        if inicio==0:
            inicio=str('Usuário')
        else:
            inicio=str('Máquina')
        print('O vencedor do sorteio para incio foi '+inicio)
        time.sleep(1)
        if inicio=='Usuário':
            print('Então você começa')
            for k in range(1,10,1):
                if k%2!=0:
                    tabuleiro=jogadausuario(nome1,tabuleiro,s1)
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('PARABÉNS,VOCÊ VENCEU')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
                elif k%2==0:
                    print('Minha vez')
                    time.sleep(2)
                    x=str(maquinainteligente2(tabuleiro,s2))
                    i=int(x[0])
                    j=int(x[1])
                    while tabuleiro[i][j]!=" ":
                        x=str(maquinainteligente2(tabuleiro,s2))
                        i=int(x[0])
                        j=int(x[1])
                    tabuleiro[i][j]=s2
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('Ahh, não foi dessa vez')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
        elif inicio=='Máquina':
            print('Então eu começo')
            for k in range(1,10,1):
                if k%2!=0:
                    print('Minha vez')
                    time.sleep(2)
                    x=str(maquinainteligente2(tabuleiro,s2))
                    i=int(x[0])
                    j=int(x[1])
                    while tabuleiro[i][j]!=" ":
                        x=str(maquinainteligente2(tabuleiro,s2))
                        i=int(x[0])
                        j=int(x[1])
                    tabuleiro[i][j]=s2
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('Ahh, não foi dessa vez')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue
                elif k%2==0:
                    tabuleiro=jogadausuario(nome1,tabuleiro,s1)
                    mostrar_tabuleiro(tabuleiro)
                    if verificar_vitoria(tabuleiro)==True:
                        print('PARABÉNS,VOCÊ VENCEU')
                        break
                    elif k==9 and verificar_vitoria(tabuleiro)==False:
                        print('DEU VELHA!!!')
                    else:
                        continue        
    começar=str(input('Você deseja jogar novamente?[S/N] '))
    while True:
        if começar=='N':
            print('Até a próxima então!')
            break
        elif começar in 'SN':
            break
        else:
            print('Resposta inválida')
            começar=str(input('Você deseja jogar novamente?[S/N] '))
            continue
