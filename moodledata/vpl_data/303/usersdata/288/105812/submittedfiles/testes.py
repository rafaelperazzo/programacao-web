...
def jogadaComputador(tabuleiro, letraComputador):
    ...
    for i in range(1, 10):
        ...
        if vazio(copy, i):
            ...
            if verificaVencedor(copy, letravoce):
                return i
...
while True:
    tabul = [' '] * 10
    letravoce, letraComputador = solicitaSimboloDoHumano()
    ...
    while rodando:
        if turn == 'voce':
            ...
            movimentacao(tabul, letravoce, movimento)

            if verificaVencedor(tabul, letravoce):
                desenhaTabuleiro(tabul)
