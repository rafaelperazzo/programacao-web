coordenadas=[0.0,0.1,0.2,1.1,1.2,1.3,2.0,2.1,2.2]
...
def jogadaHumana(tabuleiro):
    movimento = 0
    while movimento not in '[0.0,0.1,0.2,1.0,1.1,1.2,2.0,2.1,2.2]'.split() or not vazio(tabuleiro, float(tmovimento)):
...
def jogadaComputador(tabuleiro, letraComputador):
    ...
    movimento = movAleatoria(tabuleiro, [2.0,2.2,0.0,0.2])
    if movimento != None:
        return movimento

    if vazio(tabuleiro, [1.1]):
        return [1.1]
