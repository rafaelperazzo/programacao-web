...
def analisemagica(x):
    ...
    for i in range(1,len(matrizanalisecoluna)-1,1):
    ...
    if sum(matrizanalisediagonal)==sum(matrizanalisediagonal2):
        c=True
        if sum(x[0])==sum(matrizanalisediagonal) and sum(x[0])==sum(matrizcoluna[0]):
            c=True
        else:
            c=False
    else:
